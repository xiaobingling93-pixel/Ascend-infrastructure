...
Issue描述质量检测助手 是社区 Issue 的描述质量助手。它会自动评估提交内容是否“可处理”，并给出清晰反馈，帮助用户一次性补齐关键信息，减少维护者来回追问。

#### 1. 自动识别问题类型
- 自动识别 Bug、Doc、Installation、Usage等常见类型。
- 支持 Issue 场景。
- 不同类型按对应标准评估，反馈更贴合实际问题。

#### 2. 一次给出可执行反馈
- 告诉你当前描述是否基本合格。
- 指出缺失了哪些关键信息（如环境、复现条件、关键日志）。
- 给出可直接照做的补充建议，而不是泛泛而谈。
- 输出评分与结论，方便维护者快速分流处理优先级。

#### 3. 提升社区协作效率

- 让新人更容易提交高质量 Issue。
- 让维护者把时间用在问题定位和修复，而不是收集基础信息。
- 在大规模社区里保持更一致的描述评审标准。
...
```mermaid
flowchart TD
    A[提交 Issue 标题/描述] --> B[Issue描述质量检测助手 判定]
    B --> C{是否合格}
    C -->|合格| D[正常后续处理]
    C -->|不合格| E[给出缺失项与修改提示]
    E --> F[用户修改标题/描述]
...
如需扩展到新问题类型，可按社区规范补充对应模板，机器人会沿用同样的评估与反馈机制。
